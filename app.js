// TM Alpha 9.21 — 億家Pay正式扣款改為 POS 交易落地成功後才呼叫；加入付款失敗作廢與扣款後例外補償。
// TM Alpha 9.20 — 億家Pay現金儲值改為帶入正式 TM 結帳畫面收現金後再入錢包。
// TM Alpha 9.18 — 修正 SC 已設定蕃薯快速金額鍵但 TM 分類入口未重建。
// TM Alpha 9.16 — 儲值掃會員回填、蕃薯快速鍵強制同步、億家Pay付款 token 改由 RPC 驗證。
// TM Alpha 9.13 — 會員條碼／億家Pay自動帶會員：已綁定會員的小計不再重複跳確認視窗；億家Pay會員自動累點。
// TM Alpha 9.12 — 啟動白畫面修正：修正億家Pay結帳欄位 renderFields 語法錯誤。
// TM Alpha 9.11 — 億家Pay正式串接：付款碼驗證／四參數扣款／退款／現金儲值。
...
import{getCloudConfig,setCloudConfig,cloudConfigured,cloudStatus,testCloudConnection,cloudPushAll,cloudPullAll,cloudPullKey,cloudPushKey,cloudBootstrap,cloudPullPromotionRules,posFindLogisticsBatch,posFindLogisticsBatchDirect,posReceiveLogisticsBatch,posResolveReceivingCode,posGetInventoryReceiptDetails,posCompleteInventoryReceipt,posReceiveEcArrivalBatch,posListPendingLogisticsBatches,posLeaveEcReturnBatch,posEcSummary,posSearchEcPickup,posPickupEcPackage,posListEcFlow,tmRedeemAnybuy,tmRedeemAppTicket,tmRestoreAnybuyRedemption,tmSyncMemberPoints,tmGetYijiaPayCode,tmCompleteYijiaPayCode,tmRefundYijiaPayPayment,tmCompensateYijiaPayPayment,tmReloadYijiaPayWallet}from'./sync.js';

let posCategory='全部';
let yijiaPayPendingCode=null;
function yijiaPayReasonText(reason){
 const map={not_found:'付款碼不存在',expired:'付款碼已過期',used:'付款碼已使用',cancelled:'付款碼已取消',amount_required:'付款金額未提供'};
 return map[String(reason||'')]||String(reason||'付款碼無效');
}
function clearYijiaPayPending(){yijiaPayPendingCode=null}
function yijiaPayShortageMessage(r,amount){
 const balance=Number(r?.walletBalance||0),due=Number(amount||r?.amount||0),shortage=Math.max(0,Number(r?.shortage??(due-balance))||0);
 return `億家Pay餘額不足\n目前餘額：${money(balance)}\n應付金額：${money(due)}\n尚差金額：${money(shortage)}`;
}
async function acceptYijiaPayPayCode(code,{quiet=false}={}){
 // Alpha 9.16：億家Pay付款碼格式由 Supabase RPC 統一判斷。
 // 前端不再套用舊 YP 固定格式，也不改變 token 大小寫；Code128 / QR 讀到什麼就送什麼。
 const q=String(code||'').trim();
 if(!q){if(!quiet)alert('未讀取到億家Pay付款碼');return null}
 if(!cloudConfigured()){if(!quiet)alert('TM 尚未設定 Supabase，無法驗證億家Pay付款碼');return null}
 try{
  const r=await tmGetYijiaPayCode(q);
  if(!r?.ok){clearYijiaPayPending();if(!quiet)alert(yijiaPayReasonText(r?.reason||r?.status));return null}
  const expires=Date.parse(String(r.expiresAt||''));
  if(Number.isFinite(expires)&&expires<=Date.now()){clearYijiaPayPending();if(!quiet)alert('付款碼已過期');return null}
  yijiaPayPendingCode={...r,payCode:q,scannedAt:new Date().toISOString()};
  const phone=String(r.memberPhone||'').replace(/\D/g,'');
  if(phone){
   try{await refreshPosMembersCloud({redraw:false})}catch(_e){}
   const m=findPosMemberByPhone(phone)||posFindMember(String(r.memberNo||''))||posFindMember(phone);
   if(m){
    state.member=m;
    state.memberRedeemPoints=0;
    state.memberRedeemAmount=0;
    scheduleCustomerDisplaySync(0);
   }
  }
  saveAudit('億家Pay付款碼驗證',`${q}｜${r.memberPhone||''}｜餘額 ${money(Number(r.walletBalance||0))}`);
  if(!quiet){drawPOS();alert(`億家Pay付款碼驗證成功\n會員：${r.memberNo||r.memberPhone||'會員'}\n錢包餘額：${money(Number(r.walletBalance||0))}\n有效至：${r.expiresAt?new Date(r.expiresAt).toLocaleTimeString('zh-TW'): '60秒內'}`)}
  return yijiaPayPendingCode;
 }catch(e){clearYijiaPayPending();if(!quiet)alert('億家Pay付款碼驗證失敗：'+(e?.message||e));return null}
}
...
function isPosServiceItem(x){return !!(x&&(x.ecPickup===true||x.collectionPayment===true||x.billPayment===true||x.utilityPayment===true||x.yijiaPayReload===true||['bill','utility-bill','ec-pickup','collection','yijiapay-reload'].includes(String(x.serviceType||''))))}
...
async function scanCode({title='掃描條碼',onResult,onCancel}={}){
 ...
 const finish=async(code,meta={})=>{if(done)return;done=true;try{if(scanner)await scanner.stop()}catch{}genericDialog.close();if(code&&onResult)await onResult(String(code).trim(),meta);else if(!code&&onCancel)await onCancel()};
 setTimeout(async()=>{
...
async function runSubtotalFlow(){
 ...
 // Alpha 9.13：會員條碼或億家Pay付款碼已經把會員帶入本筆交易時，
 // 不再重複跳出「會員累點」確認視窗；直接進入小計，結帳後照既有規則累積點數。
 if(state.member){
  await continueSubtotalFlow();
  return;
 }
...
const APP_ANYBUY_REDEMPTIONS_KEY='yj_app_redemptions';
...
function appAnybuyMemberCodePhone(code){
 const q=String(code||'').trim().toUpperCase();
 return /^YJ09\d{8}$/.test(q)?q.slice(2):'';
}
function appAnybuyEntitlementOwnedByMember(x,memberCode){
 const q=String(memberCode||'').trim().toUpperCase(),phone=appAnybuyMemberCodePhone(q);
 return String(x?.memberNo||'').trim().toUpperCase()===q||
  String(x?.memberKey||'').trim().toUpperCase()===q||
  String(x?.memberPhone||'').trim()===phone;
}
function appAnybuyRedeemableStatus(x){
 return String(x?.status||'可兌換').trim()==='可兌換'&&Math.max(0,Number(x?.remainingQuantity||0))>0;
}
function appAnybuyEntitlementExpired(x){
 const raw=String(x?.validUntil||'').trim();if(!raw)return false;
 const t=Date.parse(raw);return Number.isFinite(t)&&Date.now()>t;
}
async function appAnybuyRedeemableForMember(memberCode){
 let rows=appMemberProductsLocal();
 if(cloudConfigured()){
  const fresh=await cloudPullKey(APP_MEMBER_PRODUCTS_KEY);
  if(Array.isArray(fresh))rows=fresh;
 }
 return rows.filter(x=>appAnybuyEntitlementOwnedByMember(x,memberCode)&&appAnybuyRedeemableStatus(x)&&!appAnybuyEntitlementExpired(x));
}
function anybuyRedemptionSaleFromRpc(result){
 const saleId=String(result?.tmSaleId||('T'+Date.now()));
 const rows=Array.isArray(result?.items)?result.items:[];
 const items=rows.map((x,i)=>{
  const qty=Math.max(1,Number(x?.quantity||1));
  const amount=Math.max(0,Number(x?.recognizedRevenueAmount||0));
  return {
   id:`ANYBUY-REDEEM-${x?.memberProductId||i}`,
   lineId:`${saleId}-L${i+1}`,
   productId:String(x?.productId||''),code:String(x?.code||''),barcode:String(x?.code||''),
   name:String(x?.name||'隨買跨店取兌換'),qty,price:qty?amount/qty:amount,cost:0,
   returnedQty:0,correctedQty:qty,appAnybuyRedeem:true,anybuyRedemption:true,
   appAnybuyMemberProductId:String(x?.memberProductId||''),appAnybuyOrderId:String(x?.orderId||''),
   appAnybuyPaymentCode:String(x?.paymentCode||''),recognizedRevenueAmount:amount,
   remainingQuantity:Number(x?.remainingQuantity||0),allowNegativeStock:true
  };
 });
 const total=Math.round(Math.max(0,Number(result?.totalRecognizedRevenue||0))*100)/100;
 return {
  id:saleId,items,ecItems:[],serviceItems:[],subtotal:total,merchandiseSubtotal:total,
  promotionDiscount:0,manualDiscount:0,discountTotal:0,discount:0,total,netTotal:total,
  payment:'隨買跨店取兌換',paymentSubtype:'原付款已完成',paymentBreakdown:[],
  cashAmount:0,nonCashAmount:0,netCashAmount:0,netNonCashAmount:0,tendered:0,change:0,
  serviceSale:false,serviceAmount:0,serviceCashAmount:0,serviceCustomerCount:0,
  deferredRevenueSale:false,deferredRevenueAmount:0,consignmentSale:false,consignmentAmount:0,consignmentNetAmount:0,
  excludeFromRevenue:false,anybuyRedemption:true,recognizedRevenueAmount:total,
  memberId:String(result?.memberId||''),memberPhone:String(result?.memberPhone||''),memberName:String(result?.memberName||'會員'),
  memberEarnedPoints:0,memberBonusPoints:0,memberAddedPoints:0,memberRedeemPoints:0,memberRedeemAmount:0,
  note:'隨買跨店取兌換｜不收款｜不累點',status:'正常',processingStatus:'兌換完成',correctionHistory:[],locked:false,
  user:String(currentUser()?.name||''),userAccount:String(currentUser()?.account||''),cashierAccount:String(currentUser()?.account||''),
  storeCode:String(result?.storeCode||currentStoreCode()),at:String(result?.redeemedAt||new Date().toISOString())
 };
}
async function openAppAnybuyRedemption(memberCode){
 const code=String(memberCode||'').trim().toUpperCase();
 if(!/^YJ09\d{8}$/.test(code))return alert('會員條碼格式不正確');
 if(state.cart?.length)return alert('目前交易中，請先完成或取消目前交易，再辦理隨買跨店取兌換');
 if(!cloudConfigured())return alert('TM 尚未設定 Supabase，無法讀取隨買跨店取商品');
 let rows=[];
 try{rows=await appAnybuyRedeemableForMember(code)}catch(err){return alert('讀取可兌換商品失敗：'+(err?.message||err))}
 if(!rows.length)return alert('此會員目前沒有可兌換的隨買跨店取商品');
 const list=rows.map((x,i)=>{
  const remain=Math.max(0,Number(x?.remainingQuantity||0));
  const maxValue=Math.max(0,Number(x?.recognizedRevenueRemaining??x?.netPaidRemaining??x?.netPaidTotal??0));
  return `<article class="item anybuy-redeem-item"><div class="info"><strong>${esc(x?.name||'隨買商品')}</strong><div class="meta">剩餘 ${remain}｜本商品尚待認列 ${money(maxValue)}${x?.validUntil?`｜有效至 ${esc(new Date(x.validUntil).toLocaleString('zh-TW'))}`:''}</div></div><label style="min-width:110px">兌換數量<input class="anybuyRedeemQty" data-id="${esc(x.id)}" type="number" min="0" max="${remain}" value="0" inputmode="numeric"></label></article>`;
 }).join('');
 dlg('隨買跨店取兌換',`<div class="notice"><b>會員 ${esc(code)}</b><br>請選擇本次要兌換的商品與數量。兌換不收款、不累點；確認後才會依原實付淨額認列營收。</div><div class="list">${list}</div><div class="checkout-amount"><small>本次預計認列營收</small><strong id="anybuyRedeemRevenue">$0</strong></div><button class="primary checkout-confirm" id="anybuyRedeemConfirm">確認兌換</button>`);
 const calc=()=>{
  let total=0;
  document.querySelectorAll('.anybuyRedeemQty').forEach(el=>{
   const row=rows.find(x=>String(x.id)===String(el.dataset.id));
   const max=Math.max(0,Number(row?.remainingQuantity||0));
   const q=Math.max(0,Math.min(max,Math.floor(Number(el.value||0))));
   if(Number(el.value||0)!==q)el.value=String(q);
   total+=appAnybuyRedemptionRevenueAmount(row,q);
  });
  const out=document.querySelector('#anybuyRedeemRevenue');if(out)out.textContent=money(Math.round(total*100)/100);
 };
 setTimeout(()=>{
  document.querySelectorAll('.anybuyRedeemQty').forEach(el=>el.addEventListener('input',calc));calc();
  const btn=document.querySelector('#anybuyRedeemConfirm');if(!btn)return;
  btn.onclick=async()=>{
   const selected=[...document.querySelectorAll('.anybuyRedeemQty')].map(el=>({memberProductId:String(el.dataset.id||''),quantity:Math.max(0,Math.floor(Number(el.value||0)))})).filter(x=>x.memberProductId&&x.quantity>0);
   if(!selected.length)return alert('請至少選擇 1 項兌換數量');
   if(!confirm('確認完成本次隨買跨店取兌換？\n確認後會立即扣除會員剩餘數量。'))return;
   btn.disabled=true;btn.textContent='兌換處理中…';
   const tmSaleId='T'+Date.now();
   try{
    const result=await tmRedeemAnybuy(code,selected,tmSaleId);
    if(!result?.ok)throw new Error('Supabase 未回傳成功結果');
    const sale=anybuyRedemptionSaleFromRpc(result),sales=load(K.sales,[]);
    if(!sales.some(x=>String(x.id)===String(sale.id))){sales.unshift(sale);save(K.sales,sales)}
    try{const fresh=await cloudPullKey(APP_MEMBER_PRODUCTS_KEY);if(Array.isArray(fresh))save(APP_MEMBER_PRODUCTS_KEY,fresh)}catch(_e){}
    saveAudit('隨買跨店取兌換',`${sale.id}｜${code}｜${sale.items.map(x=>`${x.name}×${x.qty}`).join('、')}｜認列 ${money(sale.total)}｜不收款｜不累點`);
    genericDialog.close();state.lastCompletedSale=structuredClone(sale);render('pos');
    alert(`兌換完成\n本次認列營收：${money(sale.total)}\n不收款、不累點`);
   }catch(err){btn.disabled=false;btn.textContent='確認兌換';alert(err?.message||String(err))}
  };
 },0);
}
...
function wholeAnybuyTransactionCloseEligibility(sale){
 ...
 if(saleHasNormalGoodsWithAnybuy(sale))return {allowed:false,reason:'此交易包含隨買代銷與一般商品，不能整筆退貨，請使用「部分退貨」'};
 if(anybuyRedeemLines(sale).length)return {allowed:false,reason:'隨買兌換交易不能整筆退貨，請使用「部分退貨」'};
 return appAnybuyPurchaseReturnEligibility(sale);
}
async function syncAppAnybuyRedeemRestoreAfterSaleChange(sale){
 if(!sale)return true;
 const hist=Array.isArray(sale.correctionHistory)?sale.correctionHistory:[];
 const h=hist[0];
 if(!h||!Array.isArray(h.items)||!h.items.length)return true;
 const redeemLines=anybuyRedeemLines(sale);
 if(!redeemLines.length)return true;
 let restored=0;
 for(const item of h.items){
  const line=redeemLines.find(x=>String(x.lineId||'')===String(item.lineId||''));
  if(!line)continue;
  const qty=Math.max(0,Math.floor(Number(item.qty||0)));
  if(!qty)continue;
  const amount=Math.max(0,Number(item.refund??((Number(line.price||0)*qty)||0)));
  const memberProductId=String(line.appAnybuyMemberProductId||'').trim();
  if(!memberProductId)throw new Error(`找不到 ${line.name||'隨買商品'} 的 App 商品識別碼`);
  const eventId=`${sale.id}|${h.id||h.at||'return'}|${line.lineId}`;
  const result=await tmRestoreAnybuyRedemption({
   memberProductId,quantity:qty,restoreAmount:amount,
   returnEventId:eventId,sourceSaleId:String(sale.id||'')
  });
  if(!result?.ok)throw new Error(`App 商品補回失敗：${line.name||memberProductId}`);
  restored+=qty;
 }
 if(restored>0){
  try{const fresh=await cloudPullKey(APP_MEMBER_PRODUCTS_KEY);if(Array.isArray(fresh))save(APP_MEMBER_PRODUCTS_KEY,fresh)}catch(_e){}
  saveAudit('隨買兌換退貨補回App',`${sale.id}｜補回 ${restored} 件｜不累點`);
 }
 return true;
}
...
async function syncAppAnybuyReturnAfterSaleChange(saleId){
 ...
 await syncAppAnybuyRedeemRestoreAfterSaleChange(sale);
...
function posCategoryTabs(){
 ...
 const list=[...cats];
 // Alpha 9.18：SC 只要有啟用中的快速金額鍵，TM 一定要有「蕃薯」分類入口。
 // 不要求另外在 TM 畫面分類再新增一次，避免 SC 已設定但 TM 看不到。
 const hasQuickKeys=tmQuickAmountKeys().length>0;
 const hasSweetPotato=list.some(x=>tmCategoryDisplayName(x)==='蕃薯');
 if(hasQuickKeys&&!hasSweetPotato)list.push('FF');
...
async function openAppAnybuyStorePaymentOrder(order){
 ...
 setTimeout(()=>{
  ...
  document.querySelector('#appAnybuyStorePayGo')?.addEventListener('click',async()=>{
   await refreshPosMembersCloud({redraw:false}).catch(()=>{});
   const linkedMember=ensurePosMemberFromAppOrder(order);
   if(!linkedMember)return alert('此 App 訂單缺少會員資料，無法累點，請重新整理 App 訂單後再試。');
   state.member=linkedMember;
...
const TM_ANYBUY_DEPOSIT_SYNC_QUEUE_KEY='yj_tm_anybuy_deposit_sync_queue';
function tmAnybuyProductRows(){const rows=load(TM_ANYBUY_PRODUCTS_KEY,[]);return Array.isArray(rows)?rows:[]}
function tmAnybuyDateActive(x){
 const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Taipei',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
 const start=String(x?.activityStartDate||x?.startDate||''),end=String(x?.activityEndDate||x?.endDate||'');
 return x?.active!==false&&(!start||today>=start)&&(!end||today<=end);
}
function tmAnybuyGroupPrice(x){return Math.max(0,Number(x?.price??x?.salePrice??x?.activityPrice??0))}
function tmAnybuyConfiguredGroupCount(x){return Math.max(1,Math.floor(Number(x?.groupCount??1)))}
function tmAnybuyAcquireQty(x){return Math.max(1,Math.floor(Number(x?.quantity??x?.acquiredQuantity??x?.redeemQuantity??1)))}
function tmAnybuyValidityDays(x){return Math.max(0,Math.floor(Number(x?.validityDays??x?.validDays??0)))}
async function tmAnybuyActiveProducts(){
 let rows=tmAnybuyProductRows();
 if(cloudConfigured())try{const fresh=await cloudPullKey(TM_ANYBUY_PRODUCTS_KEY);if(Array.isArray(fresh))rows=fresh}catch(e){console.warn('TM隨買活動同步失敗',e)}
 return rows.filter(tmAnybuyDateActive);
}
function tmAnybuyPhoneFromMemberCode(code){const q=String(code||'').trim().toUpperCase();return /^YJ09\d{8}$/.test(q)?q.slice(2):''}
function tmAnybuyPendingQueue(){const rows=load(TM_ANYBUY_DEPOSIT_SYNC_QUEUE_KEY,[]);return Array.isArray(rows)?rows:[]}
function queueTmAnybuyDepositSync(payload){const rows=tmAnybuyPendingQueue().filter(x=>String(x.saleId||'')!==String(payload?.saleId||''));rows.unshift({...payload,queuedAt:new Date().toISOString()});save(TM_ANYBUY_DEPOSIT_SYNC_QUEUE_KEY,rows.slice(0,100))}
function removeTmAnybuyDepositSync(saleId){save(TM_ANYBUY_DEPOSIT_SYNC_QUEUE_KEY,tmAnybuyPendingQueue().filter(x=>String(x.saleId||'')!==String(saleId||'')))}
async function finalizeTmAnybuyDeposit(pending,sale){
 const paidAt=new Date().toISOString(),saleId=String(sale?.id||pending?.saleId||'');
 try{
  if(!cloudConfigured())throw new Error('TM 尚未連線 Supabase');
  let owned=appMemberProductsLocal();
  try{const fresh=await cloudPullKey(APP_MEMBER_PRODUCTS_KEY);if(Array.isArray(fresh))owned=fresh}catch(_e){}
  const lines=Array.isArray(pending?.lines)?pending.lines:[];
  const saleNet=Math.max(0,Number(sale?.netTotal??sale?.total??0));
  const gross=lines.reduce((sum,x)=>sum+Math.max(0,Number(x.groupPrice||0))*Math.max(1,Number(x.groups||1)),0);
  let allocated=0;
  lines.forEach((x,i)=>{
   const lineGross=Math.max(0,Number(x.groupPrice||0))*Math.max(1,Number(x.groups||1));
   const netPaidTotal=i===lines.length-1?Math.max(0,Math.round((saleNet-allocated)*100)/100):Math.max(0,Math.round((gross>0?saleNet*(lineGross/gross):saleNet/Math.max(1,lines.length))*100)/100);
   allocated+=netPaidTotal;
   const acquired=Math.max(1,Math.floor(Number(x.acquiredQuantity||1)))*Math.max(1,Math.floor(Number(x.groups||1)));
   const validityDays=Math.max(0,Math.floor(Number(x.validityDays||0)));
   const productCode=String(x.code||x.productCode||'').trim();
   const entitlementId=`ENT-TM-${saleId}-${String(x.activityId||productCode||i+1)}`;
   const row={
    id:entitlementId,memberId:String(pending?.memberId||''),memberNo:String(pending?.memberCode||''),memberPhone:String(pending?.memberPhone||''),memberKey:String(pending?.memberCode||''),
    orderId:`TM-${saleId}`,paymentCode:'',productId:String(x.productId||''),code:productCode,name:String(x.name||'隨買商品'),category:String(x.category||'隨買'),
    cartQuantity:Math.max(1,Math.floor(Number(x.groups||1))),originalQuantity:acquired,remainingQuantity:acquired,netPaidTotal,netPaidRemaining:netPaidTotal,
    recognizedRevenueTotal:netPaidTotal,recognizedRevenueRemaining:netPaidTotal,recognizedRevenueAmount:0,validityDays,validFrom:paidAt,validUntil:appAnybuyValidityUntil(paidAt,validityDays),
    groupCount:Math.max(1,Math.floor(Number(x.configuredGroupCount||1))),activityStartDate:String(x.activityStartDate||''),activityEndDate:String(x.activityEndDate||''),activityContent:String(x.activityContent||''),status:'可兌換',source:'tm_counter',
    paidStoreCode:currentStoreCode(),paidStoreName:String(localStorage.getItem('yj_store_name')||'').replace(/^"|"$/g,''),tmSaleId:saleId,createdAt:paidAt,updatedAt:paidAt
   };
   const idx=owned.findIndex(v=>String(v?.id||'')===entitlementId);if(idx>=0)owned[idx]={...owned[idx],...row};else owned.unshift(row);
  });
  save(APP_MEMBER_PRODUCTS_KEY,owned);
  const ok=await cloudPushKey(APP_MEMBER_PRODUCTS_KEY,owned.slice(0,5000));if(ok===false)throw new Error('會員「我的商品」同步失敗');
  const sales=load(K.sales,[]),saved=sales.find(x=>String(x.id||'')===saleId);if(saved){saved.consignmentNetAmount=saleNet;saved.deferredRevenueAmount=saleNet;saved.excludeFromRevenue=true;saved.tmAnybuyDeposit=true;save(K.sales,sales)}
  removeTmAnybuyDepositSync(saleId);
  saveAudit('TM隨買現場寄杯',`${saleId}｜${pending?.memberCode||''}｜${lines.map(x=>`${x.name}×${x.groups}組`).join('、')}｜代銷 ${money(saleNet)}｜已寄入App`);
  return true;
 }catch(err){console.warn('TM隨買現場寄杯同步失敗',err);queueTmAnybuyDepositSync({...pending,saleId,saleNet:Number(sale?.netTotal??sale?.total??pending?.saleNet??0)});return false}
}
async function retryTmAnybuyDepositSyncQueue(){
 if(!cloudConfigured())return;for(const q of tmAnybuyPendingQueue().slice(0,10)){const fakeSale={id:q.saleId||'',netTotal:Number(q.saleNet||0),total:Number(q.saleNet||0)};const ok=await finalizeTmAnybuyDeposit(q,fakeSale);if(!ok)break}
}
async function openTmAnybuyDeposit(prefillMemberCode='',prefillGroups={}){
 if(state.cart?.length)return alert('目前已有交易商品，請先完成或取消目前交易，再進入隨買跨店取寄杯。');
 if(!cloudConfigured())return alert('TM 尚未設定 Supabase，無法辦理現場寄杯。');
 const rows=await tmAnybuyActiveProducts();
 if(!rows.length)return alert('目前沒有啟用中的 TM 現場隨買跨店取活動。\nApp 活動不會顯示在這裡。');
 const cards=rows.map(x=>{const key=String(x.id||x.code||'');const preset=Math.max(0,Math.floor(Number(prefillGroups?.[key]||0)));return `<article class="item tm-anybuy-deposit-item"><div class="info"><strong>${esc(x.name||'隨買商品')}</strong><div class="meta">${esc(x.activityContent||'TM 現場活動')}｜方案 ${tmAnybuyConfiguredGroupCount(x)}組${tmAnybuyAcquireQty(x)}杯｜每份 ${money(tmAnybuyGroupPrice(x))}${tmAnybuyValidityDays(x)?`｜效期 ${tmAnybuyValidityDays(x)} 天`:''}</div></div><label style="min-width:110px">購買份數<input class="tmAnybuyDepositQty" data-id="${esc(key)}" type="number" min="0" max="99" value="${preset}" inputmode="numeric"></label></article>`}).join('');
 const presetCode=String(prefillMemberCode||'').trim().toUpperCase();
 dlg('隨買跨店取寄杯',`<div class="notice"><b>TM 現場寄杯活動</b><br>此處只顯示 TM 現場活動；App 活動必須從 App 購買。結帳時正常收款、正常累點，但購買金額先不列日商，兌換時才認列。</div><label>App 會員條碼<div class="inline-field"><input id="tmAnybuyMemberCode" placeholder="掃描或輸入 YJ09xxxxxxxx" value="${esc(presetCode)}" autocomplete="off"><button type="button" class="button" id="tmAnybuyMemberScan">📷 掃描</button></div></label><div id="tmAnybuyMemberStatus" class="notice" style="display:${presetCode?'block':'none'};margin-top:8px">${presetCode?`已讀取會員條碼：<b>${esc(presetCode)}</b>`:''}</div><div class="list">${cards}</div><div class="checkout-amount"><small>現場應收</small><strong id="tmAnybuyDepositTotal">$0</strong></div><button class="primary checkout-confirm" id="tmAnybuyDepositGo">帶入 TM 結帳</button>`);
 setTimeout(()=>{
  const calc=()=>{let total=0;document.querySelectorAll('.tmAnybuyDepositQty').forEach(el=>{const x=rows.find(v=>String(v.id||v.code||'')===String(el.dataset.id||''));total+=tmAnybuyGroupPrice(x)*Math.max(0,Math.floor(Number(el.value||0)))});const out=document.querySelector('#tmAnybuyDepositTotal');if(out)out.textContent=money(total)};
  document.querySelectorAll('.tmAnybuyDepositQty').forEach(el=>el.addEventListener('input',calc));calc();
  const codeInput=document.querySelector('#tmAnybuyMemberCode');if(codeInput){codeInput.focus();codeInput.addEventListener('keydown',e=>{e.stopPropagation();if(e.key==='Enter'){e.preventDefault();const q=String(codeInput.value||'').trim().toUpperCase();if(!/^YJ09\d{8}$/.test(q))return alert('請掃描 App 的 YJ09 會員條碼');const st=document.querySelector('#tmAnybuyMemberStatus');if(st){st.style.display='block';st.innerHTML=`已讀取會員條碼：<b>${esc(q)}</b>`}}})}
  document.querySelector('#tmAnybuyMemberScan')?.addEventListener('click',()=>{
   const keepGroups={};document.querySelectorAll('.tmAnybuyDepositQty').forEach(el=>{keepGroups[String(el.dataset.id||'')]=Math.max(0,Math.floor(Number(el.value||0)))});
   scanCode({title:'掃描 App 會員條碼',onResult:async code=>{const q=String(code||'').trim().toUpperCase();if(!q)return;if(!/^YJ09\d{8}$/.test(q))return alert('請掃描 App 的 YJ09 會員條碼');await openTmAnybuyDeposit(q,keepGroups)}})
  });
  document.querySelector('#tmAnybuyDepositGo')?.addEventListener('click',async()=>{
   const memberCode=String(codeInput?.value||'').trim().toUpperCase(),memberPhone=tmAnybuyPhoneFromMemberCode(memberCode);if(!memberPhone)return alert('請先掃描正確的 App 會員條碼（YJ09xxxxxxxx）');
   const selected=[...document.querySelectorAll('.tmAnybuyDepositQty')].map(el=>{const x=rows.find(v=>String(v.id||v.code||'')===String(el.dataset.id||''));return {x,groups:Math.max(0,Math.floor(Number(el.value||0)))}}).filter(v=>v.x&&v.groups>0);if(!selected.length)return alert('請至少選擇 1 個 TM 現場寄杯活動');
   await refreshPosMembersCloud({redraw:false}).catch(()=>{});const member=findPosMemberByPhone(memberPhone)||ensurePosMemberFromAppOrder({memberPhone,memberName:'會員',memberNo:memberCode});if(!member)return alert(`找不到手機 ${memberPhone} 的會員資料，請先確認 App 會員與 TM 會員資料已同步。`);
   if(!state.transactionId)state.transactionId='T'+Date.now();state.member=member;state.cart=selected.map((v,i)=>({id:`TM-ANYBUY-${v.x.id||v.x.code||i}-${Date.now()}`,code:String(v.x.code||''),name:String(v.x.name||'隨買商品'),price:tmAnybuyGroupPrice(v.x),cost:0,qty:v.groups,category:'TM隨買',allowNegativeStock:true,appAnybuyPayment:true,appAnybuyDeposit:true,appAnybuyRedeem:false,pointsEligible:true,consignmentSale:true,consignmentType:'隨買跨店取',excludeFromRevenue:true,deferredRevenueAmount:tmAnybuyGroupPrice(v.x)*v.groups,tmAnybuyDeposit:true,tmAnybuyActivityId:String(v.x.id||''),tmAnybuyConfiguredGroupCount:tmAnybuyConfiguredGroupCount(v.x),tmAnybuyAcquireQty:tmAnybuyAcquireQty(v.x),tmAnybuyValidityDays:tmAnybuyValidityDays(v.x),activityContent:String(v.x.activityContent||''),activityStartDate:String(v.x.activityStartDate||v.x.startDate||''),activityEndDate:String(v.x.activityEndDate||v.x.endDate||'')}));
   window.__yjTmAnybuyPendingDeposit={memberCode,memberPhone,memberId:String(member.authUserId||member.auth_user_id||''),memberName:String(member.name||'會員'),lines:selected.map(v=>({activityId:String(v.x.id||''),productId:String(v.x.productId||v.x.product_id||''),code:String(v.x.code||''),name:String(v.x.name||'隨買商品'),category:String(v.x.category||'隨買'),groupPrice:tmAnybuyGroupPrice(v.x),groups:v.groups,configuredGroupCount:tmAnybuyConfiguredGroupCount(v.x),acquiredQuantity:tmAnybuyAcquireQty(v.x),validityDays:tmAnybuyValidityDays(v.x),activityStartDate:String(v.x.activityStartDate||v.x.startDate||''),activityEndDate:String(v.x.activityEndDate||v.x.endDate||''),activityContent:String(v.x.activityContent||'')}))};
   state.note=[state.note,'TM隨買現場寄杯'].filter(Boolean).join('｜');genericDialog.close();render('pos');setTimeout(()=>{drawPOS();alert('TM 現場寄杯活動已帶入。請按「小計」後正常結帳；收款完成才會寄入 App「我的商品」。')},0);
  });
 },0);
}
function openAnybuyServiceFlow(title,kind){
 ...
 const accounting=isDeposit?'寄杯／購買只收款並建立可兌換數量，不列入日商；實際日商於兌換時認列。':'兌換時依該商品實際淨付款金額按本次兌換數量認列日商。';
...
function posMenuGroups(){
 return [
  ...
  {id:'加值',icon:'💳',items:[['yijiapay-reload','億家Pay現金儲值'],['stored-value-topup','票證加值'],['stored-value-balance','餘額查詢']]},
  {id:'取貨',icon:'📦',page:'pickup',items:[]},
  {id:'管理',icon:'⚙️',items:[['return-menu','退貨'],['ec-flow-page','EC收發作業'],['logistics','物流簽到'],['transfer-in-page','貨單轉入']]},
  {id:'隨買跨店取',icon:'🥤',items:[['anybuy-deposit','隨買跨店取寄杯'],['easycard-deposit','悠遊卡寄杯'],['easycard-redeem','悠遊卡領取']]},
  {id:'打卡',icon:'🕘',directAction:'attendance-clock',items:[]},
  {id:'會員查詢',icon:'👤',items:[['member-lookup','輸入手機號碼'],['member-lookup','查詢'],['member-lookup','新增']]},
...
function transactionReceiptPreview(s,frontMode=false){
 ...
 return `<div class="tx-receipt-paper ${saleStatusClass(s)}">
  ...
  <div class="tx-receipt-barcode"><small>退貨條碼</small>${barcode}</div>
  <div class="tx-receipt-actions">
   <button class="button" data-tx-print="${esc(s.id)}">🖨️ 補印</button>
  </div>
  ${(s.correctionHistory||[]).length?`<details class="tx-receipt-history"><summary>退貨紀錄（${s.correctionHistory.length}）</summary>${transactionHistoryHtml(s)}</details>`:''}
 </div>`;
}
...
async function scheduleTransactionCloudRefresh(pageName){
 ...
 try{
  ...
  if(document.body.dataset.mode==='front'&&target==='transactions-front'&&currentRenderedPage==='transactions-front')render('pos');
...
function openTransactionCorrection(id,frontMode){
 ...
 dlg('部分退貨',`<p>交易：${esc(sale.id)}｜目前淨額 ${money(saleNet(sale))}</p>${saleHasAnybuy(sale)?'<div class="notice">隨買跨店取屬代銷／兌換交易；若與一般商品同筆結帳，只能在這裡做部分退貨。隨買購買商品僅限付款後 7 天內且完全未兌換才能退貨。</div>':''}<div class="tx-correction-list">${(sale.items||[]).map((x,i)=>{const baseAvailable=Math.max(0,Number(x.qty||0)-Number(x.returnedQty||0));const eligible=appAnybuyLineReturnEligibility(sale,x);const available=eligible.allowed?baseAvailable:0;const tag=x.appAnybuyRedeem?'隨買兌換':(x.appAnybuyPayment||x.appAnybuyDeposit?'隨買代銷':'');return `<label class="tx-correction-row ${eligible.allowed?'':'is-disabled'}"><span><b>${esc(x.name)}</b>${tag?`<small class="tx-anybuy-tag">${esc(tag)}</small>`:''}<small>原數量 ${x.qty}｜已退 ${x.returnedQty||0}｜可退 ${available}</small>${!eligible.allowed?`<small class="bad">${esc(eligible.reason)}</small>`:''}</span><input type="number" min="0" max="${available}" value="0" data-tx-return-qty="${x.lineId||`${sale.id}-L${i+1}`}" ${eligible.allowed?'':'disabled'}></label>`}).join('')}</div><label>退貨原因<select id="txReason">${reasonOptions.map(x=>`<option>${x}</option>`).join('')}</select></label><label>備註<input id="txReasonNote" placeholder="選填"></label><p class="tx-no-print">完成後不另外列印退貨明細。</p><button class="primary" id="txCorrectionSave">確認部分退貨</button>`);
 setTimeout(()=>{
  ...
  document.querySelector('#txCorrectionSave').onclick=async()=>{
    if(!frontMode&&!requirePermission('transactionBackCorrect'))return;
    const saveBtn=document.querySelector('#txCorrectionSave');if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='退貨同步中…'}
    ...
    try{
      const yjOriginal=load(K.sales,[]).find(x=>x.id===id)||sale;
      const yjRefundAmount=adjustments.reduce((sum,a)=>{const line=(yjOriginal.items||[]).find(x=>String(x.lineId)===String(a.lineId));return sum+Number(line?.price||0)*Number(a.returnQty||0)},0);
      correctSale(id,adjustments,txReason.value,txReasonNote.value.trim());reconcileMemberPointsAfterReturn(id);
      if(yjOriginal?.yijiaPayCode&&yjRefundAmount>0){
       try{
        const refundSaleId=`${id}-R${Date.now()}`;
        const rr=await tmRefundYijiaPayPayment(yjOriginal.yijiaPayCode,refundSaleId,yjRefundAmount,[txReason.value,txReasonNote.value.trim()].filter(Boolean).join('｜'));
        if(!rr?.ok)throw new Error(rr?.reason==='refund_exceeds_remaining'?`退款超過剩餘可退金額 ${money(Number(rr.refundableAmount||0))}`:(rr?.reason||'退款失敗'));
        const sales=load(K.sales,[]),changed=sales.find(x=>x.id===id);if(changed){changed.yijiaPayRefundedAmount=Number(changed.yijiaPayRefundedAmount||0)+Number(rr.refundAmount||0);changed.yijiaPayLastRefundSaleId=refundSaleId;save(K.sales,sales)}
        saveAudit('億家Pay退款',`${id}｜${refundSaleId}｜${money(Number(rr.refundAmount||0))}｜回錢包 ${money(Number(rr.balanceAfter||0))}`);
       }catch(payRefundErr){alert('TM退貨已完成，但億家Pay退款失敗：'+(payRefundErr?.message||payRefundErr))}
      }
      try{
       await syncAppAnybuyReturnAfterSaleChange(id);
       ...
       alert('TM 退貨已完成，但 App 商品補回同步失敗：'+(syncErr?.message||syncErr));
      }
      genericDialog.close();
      render(frontMode?'transactions-front':'transactions');
    }catch(err){if(saveBtn){saveBtn.disabled=false;saveBtn.textContent='確認部分退貨'}alert(err.message)}
  };
},0);
}
function confirmWholeTransaction(id,type,frontMode){
 const sale=load(K.sales,[]).find(x=>x.id===id);
 if(!sale)return alert('找不到交易');
 openWholeReturnRefundDialog(sale);
}
...
async function applyMemberBarcodeOnly(memberCode){
 const code=String(memberCode||'').trim().toUpperCase();
 const phone=appAnybuyMemberCodePhone(code);
 if(!phone)return alert('會員條碼格式不正確');
 try{await refreshPosMembersCloud({redraw:false})}catch(_e){}
 const member=findPosMemberByPhone(phone)||posFindMember(code)||posFindMember(phone);
 if(!member)return alert(`找不到會員 ${phone}，請先確認會員資料已同步`);
 state.member=member;state.memberRedeemPoints=0;state.memberRedeemAmount=0;
 saveAudit('會員條碼識別',`${code}｜${member.name||phone}｜僅會員識別／集點`);
 drawPOS();
}

async function redeemByAppYdCode(code){
 const q=String(code||'').trim().toUpperCase();
 if(!/^YD[0-9]+$/.test(q))return alert('YD 兌換碼格式不正確');
 if(state.cart?.length)return alert('目前交易中，請先完成或取消目前交易，再辦理隨買跨店取兌換');
 if(!cloudConfigured())return alert('TM 尚未設定 Supabase，無法讀取 App 商品兌換碼');
 const tmSaleId='T'+Date.now();
 try{
  const result=await tmRedeemAppTicket(q,tmSaleId);
  if(!result?.ok)throw new Error('Supabase 未回傳成功結果');
  const sale=anybuyRedemptionSaleFromRpc(result),sales=load(K.sales,[]);
  if(!sales.some(x=>String(x.id)===String(sale.id))){sales.unshift(sale);save(K.sales,sales)}
  try{const fresh=await cloudPullKey(APP_MEMBER_PRODUCTS_KEY);if(Array.isArray(fresh))save(APP_MEMBER_PRODUCTS_KEY,fresh)}catch(_e){}
  saveAudit('YD隨買直接兌換',`${q}｜${sale.id}｜${sale.items.map(x=>`${x.name}×${x.qty}`).join('、')}｜認列 ${money(sale.total)}｜不收款｜不累點`);
  state.lastCompletedSale=structuredClone(sale);render('pos');
  alert(`兌換完成
${sale.items.map(x=>`${x.name} × ${x.qty}`).join('、')}
本次認列營收：${money(sale.total)}
不收款、不累點`);
...
async function processUnifiedPosCode(raw,{clearInput=true,scanMeta={}}={}){
 ...
 if(!state.wasteMode&&/^YP/i.test(q)){resetUnifiedScanSession();if(clearInput&&input)input.value='';await acceptYijiaPayPayCode(q);return}
 if(!state.wasteMode&&/^YD[0-9]+$/i.test(q)){resetUnifiedScanSession();if(clearInput&&input)input.value='';await redeemByAppYdCode(q);return}
 if(!state.wasteMode&&/^YJ09\d{8}$/i.test(q)){resetUnifiedScanSession();if(clearInput&&input)input.value='';await applyMemberBarcodeOnly(q);return}
...
function openCheckoutDialog(){
 ...
 const yijiaPayReloadOnly=ecOnly&&(state.cart||[]).length>0&&(state.cart||[]).every(x=>x?.yijiaPayReload===true||String(x?.serviceType||'')==='yijiapay-reload');
 const collectionOnly=ecOnly&&!yijiaPayReloadOnly&&(state.cart||[]).some(x=>x.collectionPayment||x.billPayment||x.utilityPayment);
 ...
 dlg('TM 結帳',`
  ...
  ${yijiaPayReloadOnly?`<div class="notice ec-cash-split-notice"><b>億家Pay現金儲值</b><br>本筆儲值：<b>${money(ecAmount)}</b><br>請依正常 TM 結帳收取現金，付款完成後才會寫入億家Pay錢包。</div>`:(ecAmount>0?`<div class="notice ec-cash-split-notice"><b>${collectionOnly?'代收帳單付款':'EC 代收'}</b><br>商品應收：${money(goodsTotal)}<br>服務性應收：<b>${money(ecAmount)}</b><br>服務支付：<b>${esc(serviceMethod)}</b></div>`:'')}
 ...
 setTimeout(()=>{
  ...
  const renderFields=()=>{
   ...
   if(collectionOnly&&serviceMethod==='信用卡'){fields.innerHTML=`<div class="payment-note"><b>信用卡代收</b><br>請完成信用卡付款 ${money(ecAmount)}。</div>`;}
    ...
    fields.innerHTML=`<label>行動支付<select id="paymentSubtype">${mobileOptions.map(x=>`<option>${x}</option>`).join('')}</select></label><div id="yijiaPayCheckoutStatus"></div><p class="payment-note">非現金只支付商品 ${money(goodsTotal)}。</p>${ecCashField()}`;bindEcCash();
    const sub=document.querySelector('#paymentSubtype'),status=document.querySelector('#yijiaPayCheckoutStatus');
    const syncYp=()=>{
     if(!status)return;
     const isYp=/^億家\s*Pay$/i.test(String(sub?.value||''));
     status.innerHTML=isYp?`<div class="notice yijiapay-checkout-scan"><b>億家Pay正式錢包</b><br>${yijiaPayPendingCode?`已讀取付款碼：${esc(yijiaPayPendingCode.payCode)}｜餘額 ${money(Number(yijiaPayPendingCode.walletBalance||0))}`:'尚未讀取 App 60 秒付款碼。'}<div class="toolbar" style="margin-top:10px"><button type="button" class="primary" id="yijiaPayCheckoutScan">📷 掃描億家Pay付款碼 / QR Code</button></div></div>`:'';
     const scanBtn=document.querySelector('#yijiaPayCheckoutScan');
     if(scanBtn)scanBtn.onclick=()=>{
      genericDialog.close();
      scanCode({
       title:'掃描億家Pay付款碼 / QR Code',
       onResult:async code=>{
        const ok=await acceptYijiaPayPayCode(code,{quiet:false});
        openCheckoutDialog();
        if(!ok)setTimeout(()=>alert('請重新掃描有效的億家Pay付款碼'),80);
       },
       onCancel:()=>openCheckoutDialog()
      });
     };
    };
    sub?.addEventListener('change',syncYp);syncYp();
   }
  ...
  confirmBtn.onclick=async()=>{
   ...
   const yijiaPayAmount=(String(detail.subtype||'').replace(/\s/g,'')==='億家Pay'?Number(detail.nonCashAmount||0):0)+(String(serviceMethod||'').replace(/\s/g,'')==='億家Pay'?Number(detail.serviceNonCashAmount||0):0);
   let yijiaPayDebitResult=null,yijiaPayFresh=null,yijiaPayDebitCommitted=false;
   if(yijiaPayAmount>0){
    if(!yijiaPayPendingCode?.payCode)return alert('請先掃描 App 億家Pay 60 秒付款碼');
    yijiaPayFresh=await acceptYijiaPayPayCode(yijiaPayPendingCode.payCode,{quiet:true});
    if(!yijiaPayFresh)return alert('億家Pay付款碼已失效，請請顧客重新出示付款碼');
    if(Number(yijiaPayFresh.walletBalance||0)<yijiaPayAmount)return alert(yijiaPayShortageMessage({walletBalance:yijiaPayFresh.walletBalance},yijiaPayAmount));
    const plannedSaleId=String(state.transactionId||('T'+Date.now()));
    state.transactionId=plannedSaleId;
    detail.yijiaPayCode=yijiaPayFresh.payCode;
    detail.yijiaPayAmount=yijiaPayAmount;
    detail.yijiaPayBalanceBefore=Number(yijiaPayFresh.walletBalance||0);
    detail.yijiaPayBalanceAfter=Number(yijiaPayFresh.walletBalance||0);
   }
   ...
   try{
    ...
    const yijiaPayReloadPending=window.__yjYijiaPayReloadPending?structuredClone(window.__yjYijiaPayReloadPending):null;
    const sale=checkout(detail);
    const persistedSale=load(K.sales,[]).find(x=>String(x?.id||'')===String(sale.id||''));
    if(!persistedSale)throw new Error('POS交易未成功寫入，億家Pay未扣款');
    if(yijiaPayAmount>0){
     try{
      yijiaPayDebitResult=await tmCompleteYijiaPayCode(String(yijiaPayFresh?.payCode||detail.yijiaPayCode||''),String(sale.id||''),yijiaPayAmount);
     }catch(e){
      try{closeSale(sale.id,'void','億家Pay付款未完成','正式扣款 RPC 失敗，自動作廢 POS 交易')}catch(_voidErr){}
      throw new Error('億家Pay扣款失敗：'+(e?.message||e));
     }
     if(!yijiaPayDebitResult?.ok){
      try{closeSale(sale.id,'void','億家Pay付款未完成',String(yijiaPayDebitResult?.reason||'扣款失敗'))}catch(_voidErr){}
      if(yijiaPayDebitResult?.reason==='insufficient_balance')throw new Error(yijiaPayShortageMessage(yijiaPayDebitResult,yijiaPayAmount));
      throw new Error('億家Pay付款失敗：'+yijiaPayReasonText(yijiaPayDebitResult?.reason||yijiaPayDebitResult?.status));
     }
     yijiaPayDebitCommitted=true;
     detail.yijiaPayBalanceBefore=Number(yijiaPayDebitResult.balanceBefore||0);
     detail.yijiaPayBalanceAfter=Number(yijiaPayDebitResult.balanceAfter||0);
     sale.yijiaPayCode=String(yijiaPayFresh?.payCode||detail.yijiaPayCode||'');
     sale.yijiaPayAmount=yijiaPayAmount;
     sale.yijiaPayBalanceBefore=detail.yijiaPayBalanceBefore;
     sale.yijiaPayBalanceAfter=detail.yijiaPayBalanceAfter;
     const sales=load(K.sales,[]),stored=sales.find(x=>String(x?.id||'')===String(sale.id||''));
     if(!stored){
      try{await tmCompensateYijiaPayPayment(sale.yijiaPayCode,sale.id,yijiaPayAmount,'POS交易完成後存根遺失自動撤銷')}catch(_compErr){}
      yijiaPayDebitCommitted=false;
      throw new Error('POS交易確認失敗，億家Pay已執行自動撤銷');
     }
     Object.assign(stored,{yijiaPayCode:sale.yijiaPayCode,yijiaPayAmount:sale.yijiaPayAmount,yijiaPayBalanceBefore:sale.yijiaPayBalanceBefore,yijiaPayBalanceAfter:sale.yijiaPayBalanceAfter});
     save(K.sales,sales);
    }
    try{await applyMemberBenefitsAfterSale(sale,0,0)}catch(memberErr){console.error('會員交易後同步失敗',memberErr);saveAudit('會員交易後同步待補',`${sale.id}｜${memberErr?.message||memberErr}`)}
    state.memberRedeemPoints=0;state.memberRedeemAmount=0;
    let yijiaPayReloadResult=null,yijiaPayReloadError='';
    if(yijiaPayReloadPending){
     try{
      yijiaPayReloadResult=await tmReloadYijiaPayWallet(
       yijiaPayReloadPending.phone,
       String(sale.id||yijiaPayReloadPending.saleId||''),
       Number(yijiaPayReloadPending.amount||0),
       String(yijiaPayReloadPending.description||'億家Pay現金儲值')
      );
      if(!yijiaPayReloadResult?.ok){
       if(yijiaPayReloadResult?.reason==='cash_reload_disabled')yijiaPayReloadError='目前未開放億家Pay現金儲值';
       else if(yijiaPayReloadResult?.reason==='monthly_reload_limit_exceeded')yijiaPayReloadError=`超過本月現金儲值額度\n本月上限：${money(Number(yijiaPayReloadResult.monthlyLimit||0))}\n本月已用：${money(Number(yijiaPayReloadResult.usedThisMonth||0))}\n本月剩餘：${money(Number(yijiaPayReloadResult.remainingThisMonth||0))}`;
       else yijiaPayReloadError=String(yijiaPayReloadResult?.reason||'儲值失敗');
      }else{
       saveAudit('億家Pay現金儲值',`${sale.id}｜${yijiaPayReloadPending.phone}｜${money(Number(yijiaPayReloadResult.amount||yijiaPayReloadPending.amount||0))}｜${money(Number(yijiaPayReloadResult.balanceBefore||0))}→${money(Number(yijiaPayReloadResult.balanceAfter||0))}`);
       window.__yjYijiaPayReloadPending=null;
      }
     }catch(e){yijiaPayReloadError=String(e?.message||e)}
    }
    if(detail.yijiaPayCode){saveAudit('億家Pay正式付款',`${sale.id}｜${detail.yijiaPayCode}｜${money(detail.yijiaPayAmount)}｜${money(detail.yijiaPayBalanceBefore)}→${money(detail.yijiaPayBalanceAfter)}`);clearYijiaPayPending()}
    ...
    const tmAnybuyPending=window.__yjTmAnybuyPendingDeposit?structuredClone(window.__yjTmAnybuyPendingDeposit):null;
    let tmAnybuySyncOk=true;
    if(tmAnybuyPending){tmAnybuySyncOk=await finalizeTmAnybuyDeposit(tmAnybuyPending,sale);window.__yjTmAnybuyPendingDeposit=null;}
    genericDialog.close();
    ...
    scheduleCustomerDisplaySync(0);
    if(yijiaPayReloadPending){
     if(yijiaPayReloadError)alert(`TM 現金交易已完成，但億家Pay儲值入帳失敗：${yijiaPayReloadError}\n交易編號：${sale.id}\n請勿再次收現金。`);
     else alert(`億家Pay儲值完成\n儲值：${money(Number(yijiaPayReloadResult?.amount||yijiaPayReloadPending.amount||0))}\n儲值後餘額：${money(Number(yijiaPayReloadResult?.balanceAfter||0))}\n本月剩餘額度：${money(Number(yijiaPayReloadResult?.remainingThisMonth||0))}`);
    }
    ...
    if(tmAnybuyPending&&!tmAnybuySyncOk)alert('TM 現場收款已完成，但寄杯同步暫時失敗；已加入重試佇列，連線恢復後會再次寄入 App。');
    else if(tmAnybuyPending)alert('TM 現場寄杯完成，商品已加入會員 App「我的商品」。');
   }catch(err){
    if(yijiaPayDebitCommitted&&detail.yijiaPayCode&&detail.yijiaPayAmount>0){
     try{
      const comp=await tmCompensateYijiaPayPayment(detail.yijiaPayCode,String(state.lastCompletedSale?.id||state.transactionId||''),detail.yijiaPayAmount,'TM交易完成階段例外自動撤銷');
      if(comp?.ok)clearYijiaPayPending();
     }catch(compErr){console.error('億家Pay自動撤銷失敗',compErr)}
    }
    if(needsPaymentConnection){state.paymentConnecting=false;state.paymentConnectingMethod='';await forceCustomerDisplaySync().catch(()=>{});confirmBtn.disabled=false;confirmBtn.textContent='確認付款並完成交易'}alert(err.message)
   }
  };
 },0);
}
...
function correctionModeBanner(){
 ...
 return `<div class="pos-correction-banner">
  <strong>🔴 部分退貨模式</strong>
  <span>原交易序號：${esc(mode.saleId)}</span>
  <span>退貨條碼：${esc(mode.returnCode)}</span>
  <button class="button" data-action="cancel-correction-mode">取消退貨</button>
 </div>`;
}
...
function openCorrectionRefundDialog(){
 ...
 dlg('TM 退款確認',`
  ...
  <div class="notice"><b>部分退貨</b><br>原交易：${esc(plan.sale.id)}<br>退款方式：<b>${esc(plan.payment)}</b><br>${plan.yrReq?`YR 退貨碼：${esc(plan.yrReq.returnCode||'')}`:''}</div>
  <div class="tx-receipt-history">${rows}</div>
  <label>退貨原因<select id="refundCorrectionReason"><option>客人不要</option><option>商品瑕疵</option><option>掃描錯誤</option><option>重複刷到</option><option selected>收銀錯誤</option><option>其他</option></select></label>
 ...
 setTimeout(()=>{
  ...
  btn.onclick=async()=>{
   ...
   try{
    const reason=document.querySelector('#refundCorrectionReason')?.value||'部分退貨';
...
function ensurePosMemberFromAppOrder(order){
 const phone=String(order?.memberPhone||order?.member_phone||'').trim();
 if(!phone)return null;
 let m=findPosMemberByPhone(phone);
 if(m)return m;
 const rows=load(K.members,[]);
 const memberId=String(order?.memberId||order?.member_id||'').trim()||uid();
 const memberNo=String(order?.memberNo||order?.member_no||'').trim()||`YJ${phone}`;
 m={
  id:memberId,
  authUserId:memberId,
  auth_user_id:memberId,
  name:String(order?.memberName||order?.member_name||'會員').trim()||'會員',
  phone,
  code:memberNo,
  memberNo,
  points:0,
 ...
 rows.unshift(m);save(K.members,rows);
 return m;
}
...
function openPosMemberLookup(){
 ...
 setTimeout(()=>{
  ...
  const refreshResult=member=>{
   if(!result)return;
   if(!member){result.innerHTML='<span>找不到會員</span>';return}
...
function normalizeYijiaPayReloadMemberPhone(raw){
 const q=String(raw||'').trim().toUpperCase();
 const phone=/^YJ09\d{8}$/.test(q)?q.slice(2):String(q).replace(/\D/g,'');
 return /^09\d{8}$/.test(phone)?phone:'';
}
function openYijiaPayReloadDialog(prefillPhone=''){
 if(isSelfCheckout())return alert('自助模式不開放億家Pay現金儲值');
 const initialPhone=normalizeYijiaPayReloadMemberPhone(prefillPhone)||normalizeYijiaPayReloadMemberPhone(state.member?.phone||state.member?.code||state.member?.memberNo||'');
 dlg('億家Pay現金儲值',`<div class="notice"><b>正式錢包儲值</b><br>請先確認會員，再收取現金；TM 不會直接修改餘額，只會呼叫 Supabase 正式儲值 RPC。</div><label>會員手機<div class="inline-field"><input id="ypReloadPhone" inputmode="tel" placeholder="09xxxxxxxx" value="${esc(initialPhone)}"><button type="button" class="button" id="ypReloadScanMember">📷 掃會員</button></div></label><label>儲值金額<input id="ypReloadAmount" type="number" min="1" step="1" inputmode="numeric"></label><label>備註<input id="ypReloadDesc" value="億家Pay現金儲值"></label><button class="primary" id="ypReloadSave">已收現金，確認儲值</button>`);
 setTimeout(()=>{
  const scanBtn=document.querySelector('#ypReloadScanMember');
  if(scanBtn)scanBtn.onclick=()=>scanCode({
   title:'掃描會員條碼',
   onResult:code=>{
    const phone=normalizeYijiaPayReloadMemberPhone(code);
    if(!phone){
     alert('請掃描會員條碼或 09 開頭手機條碼');
     setTimeout(()=>openYijiaPayReloadDialog(initialPhone),220);
     return;
    }
    const m=findPosMemberByPhone(phone)||posFindMember(phone)||posFindMember(`YJ${phone}`)||posFindMember(`YJ${phone.replace(/^09/,'09')}`);
    if(m){state.member=m;state.memberRedeemPoints=0;state.memberRedeemAmount=0}
    // scanCode 會先關閉共用 dialog；稍候再重開儲值表單，避免 Safari/iPad close/open 競態造成畫面直接跳掉。
    setTimeout(()=>openYijiaPayReloadDialog(phone),220);
   },
   onCancel:()=>setTimeout(()=>openYijiaPayReloadDialog(initialPhone),220)
  });
  const saveBtn=document.querySelector('#ypReloadSave');
  if(saveBtn){
   saveBtn.textContent='帶入 TM 結帳';
   saveBtn.onclick=()=>{
    const phone=normalizeYijiaPayReloadMemberPhone(document.querySelector('#ypReloadPhone')?.value||'');
    const amount=Number(document.querySelector('#ypReloadAmount')?.value||0);
    const desc=String(document.querySelector('#ypReloadDesc')?.value||'').trim()||'億家Pay現金儲值';
    if(!phone)return alert('請輸入正確會員手機號碼');
    if(!(amount>0))return alert('請輸入儲值金額');
    if(state.cart.length)return alert('億家Pay儲值需單獨帶入結帳，請先完成或取消目前交易');
    const m=findPosMemberByPhone(phone)||posFindMember(phone)||posFindMember(`YJ${phone}`);
    if(m){state.member=m;state.memberRedeemPoints=0;state.memberRedeemAmount=0}
    const saleId=`RLD${Date.now()}`;
    state.transactionId=saleId;
    state.cart=[{
     id:`YPR-${saleId}`,code:`YPR-${phone}`,name:`億家Pay現金儲值 ${phone}`,
     price:amount,cost:0,qty:1,manualAmount:true,allowNegativeStock:true,
     yijiaPayReload:true,serviceType:'yijiapay-reload',servicePaymentMethod:'現金',pointsEligible:false
    }];
    state.discount=0;state.memberRedeemPoints=0;state.memberRedeemAmount=0;state.payment='現金';
    state.note=`億家Pay現金儲值｜${phone}｜${money(amount)}｜${desc}`;
    window.__yjYijiaPayReloadPending={phone,amount,description:desc,saleId};
    genericDialog.close();
    render('pos');
    setTimeout(()=>{drawPOS();openCheckoutDialog()},30);
   };
  }
 },0);
}
...
function posMemberPanel(){
 ...
 <div class="pos-member-title-row"><div class="pos2-pay-title">會員</div></div>
 ...
 ${m?`<div class="toolbar"><button type="button" class="button" id="posMemberClear">移除會員</button></div>`:''}</div>`;
}
...
function savePointSettings(){
 ...
 saveAudit('點數設定',`${s.earnAmount}元集${s.earnPoints}點｜TM不提供點數折抵`);
 alert('累點規則已儲存');
 render('members');
}
function memberPointSettingsPanel(){
 ...
 return `<div class="panel" style="margin-bottom:14px">
  <div class="page-head"><h3>會員累點設定</h3></div>
  ...
  <p class="setting-hint">目前規則：${s.earnAmount} 元集 ${s.earnPoints} 點。TM 不提供點數折抵；點數兌換／點數折抵由 App 發起。</p>
  <button class="primary" data-action="save-point-settings">儲存累點設定</button>
 </div>`;
}
...
async function applyMemberBenefitsAfterSale(sale,redeemPoints=0,redeemAmount=0){
 redeemPoints=0;redeemAmount=0; // Alpha 9.02: TM 禁止點數折抵，只允許累點
 ...
 // 雲端模式先只更新本機快取，避免 save() 先推 HQ 後 RPC 再加一次造成重複累點。
 if(cloudConfigured())localStorage.setItem(K.members,JSON.stringify(members));else save(K.members,members);

 // 點數正式同步：TM/SC 共用 HQ yj4_members；App 同步 app_points / app_point_transactions。
 // 隨買兌換 pointItems 為空，不會產生任何 earn/bonus 點數事件。
 if(cloudConfigured()){
  try{
   let cloudBalance=Number(m.points||0);
   const base={phone:String(m.phone||''),memberNo:String(m.code||m.memberNo||''),saleId:String(sale.id||'')};
   if(Number(redeemPoints||0)>0){
    const r=await tmSyncMemberPoints({...base,delta:-Number(redeemPoints),eventType:'redeem',sourceId:`${sale.id}:redeem`,description:`交易 ${sale.id} 點數折抵`});
    if(Number.isFinite(Number(r?.balance)))cloudBalance=Number(r.balance);
   }
   if(earned>0){
    const r=await tmSyncMemberPoints({...base,delta:earned,eventType:'earn',sourceId:`${sale.id}:earn`,description:`交易 ${sale.id} 消費累點`});
    if(Number.isFinite(Number(r?.balance)))cloudBalance=Number(r.balance);
   }
   if(bonusPoints>0){
    const r=await tmSyncMemberPoints({...base,delta:bonusPoints,eventType:'bonus',sourceId:`${sale.id}:bonus`,description:`交易 ${sale.id} 活動贈點`});
    if(Number.isFinite(Number(r?.balance)))cloudBalance=Number(r.balance);
   }
   if(Number(redeemPoints||0)>0||earned>0||bonusPoints>0){
    m.points=cloudBalance;m.updatedAt=new Date().toISOString();save(K.members,members);sale.memberPointsAfter=cloudBalance;
   }
  }catch(err){
   console.warn('會員點數跨系統同步失敗，改由既有 HQ yj4_members 同步保留本次點數',err);
...
function customerDisplayPairingUrl(target='tm'){
 ...
 base.searchParams.set('tmv','8.95');
...
async function refreshTmQuickAmountKeys(){
 const before=JSON.stringify(tmQuickAmountKeys());
 if(cloudConfigured()){
  try{
 ...
 const after=tmQuickAmountKeys();
 // Alpha 9.18：首次從 HQ 抓到快速鍵時，必須重建整個 POS 分類列，
 // 不能只 drawPOS()，否則「蕃薯」分類按鈕不會出現。
 if(currentRenderedPage==='pos'&&before!==JSON.stringify(after)){
  setTimeout(()=>render('pos'),0);
 }
 return after;
}

let tmQuickAmountRefreshPromise=null;
function ensureTmQuickAmountKeysLoaded(){
 if(tmQuickAmountRefreshPromise)return tmQuickAmountRefreshPromise;
 tmQuickAmountRefreshPromise=refreshTmQuickAmountKeys()
  .then(rows=>{
   if(currentRenderedPage==='pos'&&tmCategoryDisplayName(state.category)==='蕃薯')drawPOS();
   return rows;
  })
  .catch(e=>{console.warn('TM quick amount keys ensure failed',e);return tmQuickAmountKeys()})
  .finally(()=>{tmQuickAmountRefreshPromise=null});
 return tmQuickAmountRefreshPromise;
}
...
function selfXModeItems(page){
 if(page===1)return[
  ['self-business','1. 營業'],['self-settings','2. 設定'],
  ['self-handover','3. 交班'],['self-read-account','4. 讀帳'],
  ['self-receipt-query','5. 存根查詢'],['self-day-close','6. 日結'],
  ['self-mode-switch','7. 模式切換'],['self-device-manager','8. 裝置／周邊管理'],
  ['blank-self-9','9.'],['blank-self-10','10.']
 ];
 return[
  ['self-power-saving','11. 省電模式'],['self-language-setting','12. 多國語系'],
  ['ec-send-toggle','13. EC包裹寄件'],['ec-pickup-toggle','14. EC包裹取件'],
  ['self-transaction-print','15. 自助交易明細列印'],['self-factory-setting','16. 廠區設定'],
  ['blank-self-17','17.'],['blank-self-18','18.'],
  ['blank-self-19','19.'],['shutdown','20. 關機']
 ];
}
function selfXModeRole(emp){
 if(!emp)return '';
 if(emp.isStocktakePersonnel===true)return '盤點人員';
 if(emp.isHeadOfficePersonnel===true)return '總部人員';
 if(emp.isEngineerPersonnel===true||emp.isEngineer===true||emp.engineer===true)return '工程師';
 const text=[emp.role,emp.title,emp.jobTitle,emp.employeeType,emp.employmentType,emp.identityType,emp.position].filter(Boolean).join('｜');
 if(/創辦人/.test(text))return '創辦人';
 if(/總部/.test(text))return '總部人員';
 if(/工程/.test(text))return '工程師';
 if(/盤點/.test(text))return '盤點人員';
 return String(emp.role||'');
}
async function selfXModeResolveAccount(account){
 const q=String(account||'').trim().toLowerCase();if(!q)return null;
 if(cloudConfigured()){
  try{await cloudPullKey(K.employees)}catch(_e){}
 }
 let emp=(employees()||[]).find(x=>tmEmployeeActive(x)&&String(x.account||'').trim().toLowerCase()===q)||null;
 if(emp)return emp;
 if(cloudConfigured()){
  try{
   const rows=await cloudPullKey('yj_stocktake_personnel_auth');
   const row=(Array.isArray(rows)?rows:[]).find(x=>x?.active!==false&&String(x.account||x.employeeNo||x.employeeCode||'').trim().toLowerCase()===q);
   if(row)return {...row,account:String(row.account||row.employeeNo||row.employeeCode||q),role:String(row.role||'盤點人員'),isStocktakePersonnel:true,active:true,employmentStatus:'在職'};
  }catch(_e){}
 }
 return null;
}
function selfXModeAuthorize(title,allowedRoles,onSuccess){
 dlg(`${title}｜帳號驗證`,`
  <div class="tm-mode-auth-card self-x-auth-card">
   <h3>${esc(title)}</h3>
   <p>請輸入授權帳號。</p>
   <div class="notice">允許身分：<b>${allowedRoles.map(esc).join('、')}</b></div>
   <label>帳號<input id="selfXAuthAccount" autocomplete="username" placeholder="請輸入帳號"></label>
   <div id="selfXAuthState" class="notice">尚未驗證</div>
   <div class="tm-mode-auth-actions"><button class="button" id="selfXAuthCancel">取消</button><button class="primary" id="selfXAuthGo">確認</button></div>
  </div>`);
 setTimeout(()=>{
  const input=document.querySelector('#selfXAuthAccount'),stateEl=document.querySelector('#selfXAuthState'),goBtn=document.querySelector('#selfXAuthGo');input?.focus();let busy=false;
  const go=async()=>{if(busy)return;busy=true;if(goBtn)goBtn.disabled=true;try{
   const emp=await selfXModeResolveAccount(input?.value||'');
   if(!emp){if(stateEl)stateEl.textContent='找不到有效帳號';return}
   const role=selfXModeRole(emp);
   if(!allowedRoles.includes(role)){if(stateEl)stateEl.textContent=`權限不足：${role||'一般人員'}不可操作`;return}
   tmSelfManagerEmployee=emp;saveAudit(`自助 XMode ${title}授權`,`${emp.name||emp.account}｜${emp.account}｜${role}`);genericDialog.close();onSuccess?.(emp);
  }finally{busy=false;if(goBtn)goBtn.disabled=false}};
  goBtn?.addEventListener('click',go);input?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();go()}});
  document.querySelector('#selfXAuthCancel')?.addEventListener('click',()=>openTmModePanel('X',tmModePage));
 },0);
}
function openSelfXSettings(){
 dlg('自助模式設定',`<div class="self-x-submenu"><button class="tm-mode-item" data-self-x-sub="display">客顯配對</button><button class="tm-mode-item" data-self-x-sub="ecsend">EC包裹寄件設定</button><button class="tm-mode-item" data-self-x-sub="ecpickup">EC包裹取件設定</button><button class="tm-mode-item" data-self-x-sub="factory">廠區設定</button><button class="tm-mode-item" data-self-x-sub="back">返回 XMode</button></div>`);
 setTimeout(()=>document.querySelectorAll('[data-self-x-sub]').forEach(b=>b.addEventListener('click',()=>{
  const a=b.dataset.selfXSub;if(a==='display')return openCustomerDisplayPairing();if(a==='ecsend')return openSelfEcToggleAuthorization('ecSend','EC寄件',true);if(a==='ecpickup')return openSelfEcToggleAuthorization('ecPickup','EC取貨',true);if(a==='factory')return openTmFactorySetting();openTmModePanel('X',1);
 })),0);
}
function openSelfXDeviceManager(){
 dlg('裝置／周邊管理',`<div class="self-x-submenu"><button class="tm-mode-item" id="selfXDeviceDisplay">客顯配對</button><button class="tm-mode-item" id="selfXDevicePrinter">印表／小票設備</button><button class="tm-mode-item" id="selfXDeviceBack">返回 XMode</button></div>`);
 setTimeout(()=>{document.querySelector('#selfXDeviceDisplay')?.addEventListener('click',()=>openCustomerDisplayPairing());document.querySelector('#selfXDevicePrinter')?.addEventListener('click',()=>dlg('印表／小票設備','<p>目前沿用既有 TM 印表設備設定，不改動一般 TM。</p><button class="primary" id="selfXPrinterBack">返回</button>'));document.querySelector('#selfXDeviceBack')?.addEventListener('click',()=>openTmModePanel('X',1));},0);
}
function openSelfXLanguage(){
 dlg('多國語系',`<div class="panel"><h3>自助模式語言</h3><button class="primary" id="selfXLangZh">繁體中文</button><button class="button" disabled>English（預留）</button><button class="button" id="selfXLangBack">返回 XMode</button></div>`);
 setTimeout(()=>{document.querySelector('#selfXLangZh')?.addEventListener('click',()=>{alert('目前使用繁體中文');openTmModePanel('X',2)});document.querySelector('#selfXLangBack')?.addEventListener('click',()=>openTmModePanel('X',2));},0);
}
...
function openTmModePanel(modeName='X',page=1,options={}){
 ...
 const selfX=tmModeName==='X'&&isSelfCheckout();
 tmModePage=Math.max(1,Math.min(selfX?2:3,Number(page)||1));
 if(selfX){
  dlg('自助模式 XMode',`
   <div class="tm-mode-panel self-xmode-panel">
    <div class="tm-mode-head self-xmode-head"><strong>XMode</strong><div><span>自助結帳｜99999</span><span>店號：${esc(currentStoreCode())}｜版本：${esc(tmDisplayVersion())}</span></div></div>
    <div class="tm-mode-grid self-xmode-grid">${selfXModeItems(tmModePage).map(([a,l])=>`<button type="button" class="tm-mode-item ${a.startsWith('blank-')?'is-blank':''}" data-tm-mode-action="${esc(a)}" ${a.startsWith('blank-')?'disabled':''}>${esc(tmModeButtonLabel(a,l))}</button>`).join('')}</div>
    <div class="tm-mode-pager self-xmode-pager"><button type="button" class="tm-mode-arrow" id="tmModePrev" ${tmModePage<=1?'disabled':''}>◀</button><strong>${tmModePage} / 2</strong><button type="button" class="tm-mode-arrow" id="tmModeNext" ${tmModePage>=2?'disabled':''}>▶</button><button type="button" class="tm-mode-exit" id="tmModeExit">離開</button></div>
    <div class="tm-mode-hint" id="tmModeHint">自助模式 XMode｜一般 TM 選單未變更</div>
   </div>`);
  setTimeout(()=>{document.querySelector('#tmModePrev')?.addEventListener('click',()=>openTmModePanel('X',tmModePage-1,options));document.querySelector('#tmModeNext')?.addEventListener('click',()=>openTmModePanel('X',tmModePage+1,options));document.querySelector('#tmModeExit')?.addEventListener('click',()=>{genericDialog.close();render('pos')});document.querySelectorAll('[data-tm-mode-action]').forEach(btn=>btn.addEventListener('click',()=>runTmModeAction(btn.dataset.tmModeAction)));},0);
  return;
 }
 dlg(`${tmModeName} Mode`,`
...
function runTmModeAction(action){
 ...
 if(isSelfCheckout()&&tmModeName==='X'){
  const privileged=['創辦人','總部人員','工程師'];
  if(action==='self-business'){selfXModeAuthorize('營業',privileged,()=>{selfCheckoutScreen='home';render('pos')});return}
  if(action==='self-settings'){selfXModeAuthorize('設定',privileged,()=>openSelfXSettings());return}
  if(action==='self-power-saving'){selfXModeAuthorize('省電模式',privileged,()=>toggleTmPowerSaving());return}
  if(action==='self-read-account'){selfXModeAuthorize('讀帳',['創辦人','盤點人員'],()=>tmModeReadAccount());return}
  if(action==='self-handover'){genericDialog.close();triggerPosAction('handover');return}
  if(action==='self-receipt-query'){genericDialog.close();render('transactions-front');return}
  if(action==='self-day-close'){genericDialog.close();triggerPosAction('manual-close');return}
  if(action==='self-mode-switch'){openTmOperationModeSwitch();return}
  if(action==='self-device-manager'){openSelfXDeviceManager();return}
  if(action==='self-language-setting'){openSelfXLanguage();return}
  if(action==='self-transaction-print'){genericDialog.close();render('transactions-front');return}
  if(action==='self-factory-setting'){openTmFactorySetting();return}
 }
...
function findTmReturnSale(code){
 const q=String(code||'').trim();
 if(!q)return null;
 return findSaleByReturnCode(q)||load(K.sales,[]).find(x=>String(x.id||'')===q)||null;
}
function openTmReturnMenu(){
 if(state.cart?.length)return alert('交易中無法辦理退貨，請先完成或取消目前交易');
 dlg('退貨',`<div class="panel" style="margin:0">
  <h3>選擇退貨方式</h3>
  <div class="pos-confirmed-flyout-grid" style="grid-template-columns:1fr 1fr">
   <button class="button" id="tmPartialReturnEntry">部分退貨</button>
   <button class="button danger" id="tmWholeReturnEntry">整筆退貨</button>
  </div>
  <div class="notice" style="margin-top:12px">部分退貨已整合原「交易更正」與「換貨」流程；換貨不再提供獨立按鍵。</div>
 </div>`);
 setTimeout(()=>{
  document.querySelector('#tmPartialReturnEntry')?.addEventListener('click',()=>openTmReturnLookup('partial'));
  document.querySelector('#tmWholeReturnEntry')?.addEventListener('click',()=>openTmReturnLookup('whole'));
 },0);
}
function openTmReturnLookup(kind='partial'){
 const whole=kind==='whole',title=whole?'整筆退貨':'部分退貨';
 dlg(title,`<div class="panel" style="margin:0">
  <h3>${title}</h3>
  <label>退貨條碼／交易序號<input id="tmReturnLookupCode" placeholder="掃描 RTT... 或輸入 T..."></label>
  <div class="toolbar">
   <button class="button" id="tmReturnLookupScan">📷 掃描</button>
   <button class="primary" id="tmReturnLookupOpen">下一步</button>
  </div>
  <div class="notice" style="margin-top:10px">${whole?'整筆退貨會退回目前交易全部可退金額。':'部分退貨可選擇原交易中的退貨品項與數量。'}</div>
 </div>`);
 const resolve=async raw=>{
  const q=String(raw||'').trim();if(!q)return alert('請輸入或掃描退貨條碼／交易序號');
  if(/^YR[A-Z0-9_-]+$/i.test(q)){
   if(whole)return alert('YR 為 App 指定商品退貨申請，請使用「部分退貨」辦理');
   genericDialog.close();
   await processUnifiedPosCode(q,{clearInput:false});
   return;
  }
  const sale=findTmReturnSale(q);
  if(!sale)return alert('找不到原交易，請確認退貨條碼或交易序號');
  genericDialog.close();
  if(whole)openWholeReturnRefundDialog(sale);
  else openSaleInCorrectionMode(sale);
 };
 setTimeout(()=>{
  document.querySelector('#tmReturnLookupOpen')?.addEventListener('click',()=>resolve(document.querySelector('#tmReturnLookupCode')?.value));
  document.querySelector('#tmReturnLookupCode')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();resolve(e.currentTarget.value)}});
  document.querySelector('#tmReturnLookupScan')?.addEventListener('click',()=>{
   scanCode({title:`掃描${title}條碼`,onResult:(code)=>{genericDialog.close();resolve(code)}})
  });
 },0);
}
function openWholeReturnRefundDialog(sale){
 if(!sale)return alert('找不到交易');
 if(isEcServiceSale(sale))return alert('EC 包裹取貨不可退貨');
 if(sale.locked)return alert('此交易已鎖定');
 const eligibility=wholeAnybuyTransactionCloseEligibility(sale);
 if(!eligibility.allowed)return alert(eligibility.reason);
 const refund=Math.max(0,Number(saleNet(sale)||0));
 if(refund<=0)return alert('此交易目前沒有可退款金額');
 const payment=[sale.payment,sale.paymentSubtype].filter(Boolean).join('｜')||'原付款方式';
 dlg('TM 退款確認',`
  <div class="checkout-amount"><small>本筆應退</small><strong>${money(refund)}</strong></div>
  <div class="notice"><b>整筆退貨</b><br>原交易：${esc(sale.id)}<br>退款方式：<b>${esc(payment)}</b></div>
  <label>退貨原因<select id="wholeReturnReason"><option>客人不要</option><option>商品瑕疵</option><option>收銀錯誤</option><option>重複刷到</option><option>店長核准</option><option>其他</option></select></label>
  <label>備註<input id="wholeReturnNote" placeholder="選填"></label>
  <div class="notice">此畫面為退款，不會再次向客人收款。</div>
  <button class="primary checkout-confirm" id="wholeReturnRefundConfirm">確認退款 ${money(refund)}</button>`);
 setTimeout(()=>{
  const btn=document.querySelector('#wholeReturnRefundConfirm');if(!btn)return;
  btn.onclick=async()=>{
   if(btn.disabled)return;btn.disabled=true;btn.textContent='退款處理中…';
   try{
    const reason=document.querySelector('#wholeReturnReason')?.value||'整筆退貨';
    const note=document.querySelector('#wholeReturnNote')?.value?.trim()||'';
    closeSale(sale.id,'return',reason,note);
    reconcileMemberPointsAfterReturn(sale.id);
    await syncAppAnybuyReturnAfterSaleChange(sale.id);
    if(sale.yijiaPayCode){
     const refundSaleId=`${sale.id}-FULL-R${Date.now()}`;
     const rr=await tmRefundYijiaPayPayment(sale.yijiaPayCode,refundSaleId,refund,[reason,note].filter(Boolean).join('｜'));
     if(!rr?.ok)throw new Error(rr?.reason==='refund_exceeds_remaining'?`億家Pay只剩 ${money(Number(rr.refundableAmount||0))} 可退款`:(rr?.reason||'億家Pay退款失敗'));
     saveAudit('億家Pay整筆退款',`${sale.id}｜${refundSaleId}｜${money(Number(rr.refundAmount||0))}｜餘額 ${money(Number(rr.balanceAfter||0))}`);
    }
    genericDialog.close();
    alert(`退款完成｜應退 ${money(refund)}｜${payment}${sale.yijiaPayCode?'｜已退回億家Pay錢包':''}`);
    render('pos');
   }catch(err){
    btn.disabled=false;btn.textContent=`確認退款 ${money(refund)}`;alert(err?.message||err)
   }
  };
 },0);
}
...
function runPosSubaction(action){
 if(action==='return-menu'){openTmReturnMenu();return}
 if(action==='tm-mode-switch'){openTmModeSwitch();return}
 if(action==='yijiapay-reload'){triggerPosAction('yijiapay-reload');return}
 ...
 if(action==='anybuy-deposit'){openTmAnybuyDeposit();return}
...
function bind(p){
 ...
 if(p==='pos')setTimeout(()=>refreshTmQuickAmountKeys().then(()=>{if(currentRenderedPage==='pos')drawPOS()}).catch(()=>{}),700);
 else if(p!=='members')setTimeout(()=>refreshTmQuickAmountKeys().catch(()=>{}),700);
...
function drawPOS(){if(isSelfCheckout()){scheduleCustomerDisplaySync(0);return}
 ...
 if(pg){
  ...
  const quickRows=quickCategoryName==='蕃薯'?tmQuickAmountKeys():[];
  if(quickCategoryName==='蕃薯'&&!quickRows.length&&cloudConfigured())setTimeout(()=>ensureTmQuickAmountKeysLoaded(),0);
  const quickHtml=quickCategoryName==='蕃薯'
   ?quickRows.map(x=>`<button class="pos-original-product tm-quick-category-key" data-tm-quick-key="${esc(x.id)}"><strong>${esc(x.name||'蕃薯')}</strong><b>${money(x.amount||0)}</b><small>快速金額</small></button>`).join('')
   :'';
...
document.body.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;
 ...
 if(b.dataset.action==='scan'){scanCode({title:state.wasteMode?'掃描廢棄商品':'掃描商品／會員／EC／代收條碼／QR Code',onResult:(code,meta)=>processUnifiedPosCode(code,{clearInput:false,scanMeta:meta})})}
 ...
 if(b.dataset.action==='logistics'){
  ...
  setTimeout(()=>{
   ...
   const findBatch=async()=>{
    ...
    try{
     // TM Alpha 8.95：YJB 正式貨單直接查 logistics_batches，不再先繞進貨條碼解析 RPC。
     const isYjb=/^YJB/i.test(code);
     let resolved=null;
     let row=null;
     if(isYjb){
      row=await posFindLogisticsBatchDirect(code);
      if(!row){resultEl.innerHTML=`<b>${esc(code)}</b><br>找不到此門市正式物流貨單。`;return}
      resolved={batch_no:String(row.batch_no||code).trim(),delivery_type:String(row.delivery_type||'').trim(),receipt_no:''};
     }else{
      resolved=await posResolveReceivingCode(code);
     ...
     const typeZh=deliveryTypeZh(row?.delivery_type||resolved.delivery_type||meta?.cloudDeliveryType||meta?.deliveryType||order?.deliveryType||'');
     if(!row)row=await posFindLogisticsBatch(resolved.batch_no,typeZh);
     ...
     if(!['pending','expected','transmitted','待簽到','未到店'].includes(rowStatus)){
      resultEl.innerHTML=`<b>${esc(row.batch_no)}</b><br>目前狀態：${esc(logisticsStatusZh(row.status||rowStatus))}<br>此貨單不是待簽到狀態。`;return
     }
     const merged=!!meta?.mergedShipment;
     ...
     document.querySelector('#ltConfirm').onclick=async()=>{
      ...
      try{
       const rpcType=String(row?.delivery_type||resolved?.delivery_type||meta?.cloudDeliveryType||meta?.deliveryType||order?.deliveryType||typeZh||'').trim();
       const received=await posReceiveLogisticsBatch(row.batch_no,rpcType,currentUser()?.name||'');
   ...
   document.querySelector('#ltScan').onclick=()=>scanCode({title:'掃描物流／進貨條碼',onResult:code=>{
    // TM Alpha 9.19：掃碼器與物流簽到共用 genericDialog。
    // 掃描成功時 scanCode 會先關閉掃碼視窗，因此原本物流簽到 DOM 已被替換。
    // 重新開啟物流簽到視窗，再把掃到的完整條碼帶回並自動查詢，避免掃完直接跳掉。
    const reopen=document.querySelector('[data-action=\"logistics\"]');
    if(reopen){
     reopen.click();
     setTimeout(()=>{
      const input=document.querySelector('#ltBatch');
      const find=document.querySelector('#ltFind');
      if(input)input.value=String(code||'').trim();
      if(find)find.click();
     },120);
    }else{
     alert('已掃描：'+String(code||'').trim()+'\n請重新開啟物流到店簽到。');
    }
   }});
 ...
 if(b.dataset.action==='yijiapay-reload'){
  openYijiaPayReloadDialog();
  return;
 }
...
async function startEnterprise(){
 // Alpha 9.07：TM 介面先啟動，雲端同步改成背景執行。
 // 避免 iOS / 行動網路下 Supabase bootstrap 卡住時整頁白畫面。
 try{seed()}catch(e){console.error('seed failed',e)}
 try{applyTmOperationMode();applyTmPowerSaving()}catch(e){console.error('local mode init failed',e)}

 if('serviceWorker'in navigator){
  navigator.serviceWorker.register('./sw.js?v='+encodeURIComponent(window.APP_VERSION||window.YJ_VERSION||'9.07'),{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
 }
 ...
 try{
  ...
  try{sessionStorage.removeItem('yj_tm_boot_retry_905')}catch(_e){}
 }catch(e){
  console.error('TM local startup failed',e);
  document.body.innerHTML='<main style="min-height:100dvh;display:grid;place-items:center;background:#eef7f1;padding:24px;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif"><div style="width:min(560px,100%);background:#fff;border:1px solid #cfe2d7;border-radius:22px;padding:28px;text-align:center"><h1 style="color:#087942">TM 啟動失敗</h1><p>本機介面初始化失敗，請重新整理頁面。</p><button onclick="location.reload()" style="border:0;border-radius:14px;padding:14px 24px;background:#087942;color:#fff;font-size:18px;font-weight:900">重新整理</button></div></main>';
  return;
 }

 // 雲端初始化不得阻塞 TM 首頁。最多等 12 秒後放棄本次 bootstrap，之後各功能仍可自行同步。
 const cloudBoot=Promise.resolve().then(()=>cloudBootstrap());
 const cloudTimeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('cloud bootstrap timeout')),12000));
 Promise.race([cloudBoot,cloudTimeout])
  .then(()=>{
   window.__YJ_CLOUD_BOOT_OK=true;
   try{if(document.body.dataset.mode==='front'&&document.querySelector('.pos-classic'))render('front')}catch(_e){}
  })
  .catch(e=>{console.warn('cloudBootstrap background failed',e);window.__YJ_CLOUD_BOOT_OK=false});
...
setTimeout(()=>{retryAppAnybuyPaymentSyncQueue().catch(()=>{});retryTmAnybuyDepositSyncQueue().catch(()=>{})},5000);
setInterval(()=>{retryAppAnybuyPaymentSyncQueue().catch(()=>{});retryTmAnybuyDepositSyncQueue().catch(()=>{})},60000);
