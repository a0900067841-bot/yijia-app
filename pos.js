...
export function correctSale(saleId,adjustments,reason,note=''){
 ...
 for(const a of adjustments||[]){
  ...
  const amount=round2(Number(line.price||0)*q*ratio);
  line.returnedQty=already+q;line.correctedQty=Math.max(0,Number(line.qty||0)-Number(line.returnedQty||0));
  refund=round2(refund+amount);changed.push({lineId:line.lineId,name:line.name,qty:q,refund:amount});
  restoreInventoryForReturn(sale,line,q);
 }
...
export function closeSale(saleId,type,reason,note=''){
 ...
 sale.items=(sale.items||[]).map((line,i)=>{if(!line.lineId)line.lineId=`${sale.id}-L${i+1}`;const remain=Math.max(0,Number(line.qty||0)-Number(line.returnedQty||0));if(remain>0)restoreInventoryForReturn(sale,line,remain);line.returnedQty=Number(line.qty||0);line.correctedQty=0;return line});
...
export function checkout(paymentDetail=null){
 ...
 const sale={
  ...
  excludeFromRevenue:serviceSale,
