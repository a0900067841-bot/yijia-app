...
export async function tmSyncMemberPoints({phone='',memberNo='',delta=0,eventType='earn',sourceId='',description='',saleId=''}={}){
  const c=getCloudConfig();
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  const payload={
    ...
    p_event_type:String(eventType||'earn').trim(),
    p_source_id:String(sourceId||'').trim(),
    p_description:String(description||'').trim(),
    p_sale_id:String(saleId||'').trim()
  };
  const res=await fetch(`${c.url}/rest/v1/rpc/tm_sync_member_points`,{
    method:'POST',headers:headers(),body:JSON.stringify(payload),cache:'no-store'
  });
  if(!res.ok){const text=await res.text().catch(()=> '');throw new Error(`點數同步失敗：${text||res.statusText}`)}
  const text=await res.text();return text?JSON.parse(text):null;
}

export async function tmRedeemAnybuy(memberCode,items,tmSaleId=''){
  const c=getCloudConfig();
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  const payload={
    p_member_code:String(memberCode||'').trim(),
    p_items:Array.isArray(items)?items:[],
    p_store_code:runtimeStoreId(),
    p_tm_sale_id:String(tmSaleId||'').trim()
  };
  const res=await fetch(`${c.url}/rest/v1/rpc/tm_redeem_anybuy`,{
    method:'POST',
    headers:headers(),
  ...
  if(!res.ok){
    const text=await res.text().catch(()=> '');
    throw new Error(`隨買兌換失敗：${text||res.statusText}`);
  }
  const text=await res.text();
  return text?JSON.parse(text):null;
}

export async function tmRedeemAppTicket(redeemCode,tmSaleId=''){
  const c=getCloudConfig();
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  const payload={
    p_redeem_code:String(redeemCode||'').trim().toUpperCase(),
    p_store_code:runtimeStoreId(),
    p_tm_sale_id:String(tmSaleId||'').trim()
  };
  const res=await fetch(`${c.url}/rest/v1/rpc/tm_redeem_app_ticket`,{
    method:'POST',headers:headers(),body:JSON.stringify(payload),cache:'no-store'
  });
  if(!res.ok){const text=await res.text().catch(()=> '');throw new Error(`YD 兌換失敗：${text||res.statusText}`)}
  const text=await res.text();return text?JSON.parse(text):null;
}

export async function tmRestoreAnybuyRedemption({memberProductId,quantity,restoreAmount,returnEventId,sourceSaleId=''}){
  const c=getCloudConfig();
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  const payload={
    p_member_product_id:String(memberProductId||'').trim(),
    p_quantity:Math.max(0,Math.floor(Number(quantity||0))),
    p_restore_amount:Math.max(0,Number(restoreAmount||0)),
    p_store_code:runtimeStoreId(),
    p_return_event_id:String(returnEventId||'').trim(),
    p_source_sale_id:String(sourceSaleId||'').trim()
  };
  const res=await fetch(`${c.url}/rest/v1/rpc/tm_restore_anybuy_redemption`,{
    method:'POST',headers:headers(),body:JSON.stringify(payload),cache:'no-store'
  });
  if(!res.ok){const text=await res.text().catch(()=> '');throw new Error(`隨買退貨補回 App 失敗：${text||res.statusText}`)}
  const text=await res.text();return text?JSON.parse(text):null;
}
...
function normalizeLogisticsDeliveryType(value){
  ...
  const map={
    ...
    '鮮食二配':'fresh_2',
    '乳品':'dairy',
    // TM Alpha 9.17：低溫一配正式使用 low_1。
    // 不再把後端已回傳的 low_1 強制改成 dairy，避免物流簽到 RPC 因配送別不一致而找不到貨單。
    '低溫一配':'low_1',
    '低溫二配':'low_2',
    '冷凍':'frozen',
    '億家通':'yijiatong',
...
function logisticsDeliveryTypeCandidates(value){
  const raw=String(value||'').trim();
  const primary=normalizeLogisticsDeliveryType(raw);
  const out=[];
  const add=v=>{v=String(v||'').trim();if(v&&!out.includes(v))out.push(v)};
  add(primary);
  // 相容舊資料：過去低溫一配曾以 dairy 儲存；新版則可能是 low_1。
  if(primary==='low_1')add('dairy');
  if(primary==='dairy')add('low_1');
  return out;
}

// Alpha 3.8 - 億家物流批次／POS 到店簽到 RPC
// TM Alpha 8.95 - YJB 正式貨單直連查詢。
// 掃到 YJB 時不先經過 pos_resolve_receiving_code / pos_get_logistics_batch，
// 直接以 logistics_batches.batch_no + store_code 取得正式批次。
export async function posFindLogisticsBatchDirect(batchNo){
  const c=getCloudConfig();
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  const batch=String(batchNo||'').trim();
  const store=String(c.storeId||'001').trim()||'001';
  if(!batch)return null;

  // TM Alpha 8.96：RLS 被擋時 Supabase REST 可能回 [] 而不是丟錯誤。
  // 因此「查無資料」也必須繼續走管理 RPC，不能直接當成貨單不存在。
  let directError=null;
  try{
    const rows=await request(`logistics_batches?select=*&batch_no=eq.${encodeURIComponent(batch)}&store_code=eq.${encodeURIComponent(store)}&limit=1`,{cache:'no-store'});
  ...
  try{
    const rows=await request('rpc/admin_list_logistics_batches',{
      method:'POST',
      body:JSON.stringify({p_store_code:store,p_limit:500}),
      cache:'no-store'
    });
    const list=Array.isArray(rows)?rows:[];
    const found=list.find(x=>String(x?.batch_no||'').trim()===batch)||null;
    if(found)return found;
  }catch(err){
    if(!directError)directError=err;
  }

  if(directError)console.warn('TM YJB 直連查詢失敗，表層/RPC 都未取得貨單',directError);
  return null;
}
...
export async function posFindLogisticsBatch(batchNo,deliveryType){
  ...
  const rows=await request('rpc/pos_get_logistics_batch',{
    ...
    body:JSON.stringify({
      ...
      p_delivery_type:normalizeLogisticsDeliveryType(deliveryType),
...
export async function posReceiveLogisticsBatch(batchNo,deliveryType,operatorName=''){
  ...
  const batch=String(batchNo||'').trim();
  const store=String(c.storeId||'001').trim()||'001';
  const receivedBy=String(operatorName||'').trim();
  const candidates=logisticsDeliveryTypeCandidates(deliveryType);
  let lastError=null;
...
// TM Alpha 9.11 — 億家Pay正式共用錢包 RPC
export async function tmGetYijiaPayCode(payCode){
  if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  return await request('rpc/tm_get_yijiapay_pay_code',{method:'POST',body:JSON.stringify({p_pay_code:String(payCode||'').trim()})});
}
export async function tmCompleteYijiaPayCode(payCode,saleId,amount){
  const c=getCloudConfig();if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  return await request('rpc/tm_complete_yijiapay_pay_code',{method:'POST',body:JSON.stringify({p_pay_code:String(payCode||'').trim(),p_store_code:String(c.storeId||'001').trim()||'001',p_sale_id:String(saleId||'').trim(),p_amount:Number(amount||0)})});
}
export async function tmCompensateYijiaPayPayment(payCode,saleId,amount,reason=''){
  const c=getCloudConfig();if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  return await request('rpc/tm_compensate_yijiapay_payment',{method:'POST',body:JSON.stringify({p_pay_code:String(payCode||'').trim(),p_store_code:String(c.storeId||'001').trim()||'001',p_sale_id:String(saleId||'').trim(),p_amount:Number(amount||0),p_reason:String(reason||'').trim()||null})});
}
export async function tmRefundYijiaPayPayment(payCode,refundSaleId,amount,reason=''){
  const c=getCloudConfig();if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  return await request('rpc/tm_refund_yijiapay_payment',{method:'POST',body:JSON.stringify({p_pay_code:String(payCode||'').trim(),p_store_code:String(c.storeId||'001').trim()||'001',p_refund_sale_id:String(refundSaleId||'').trim(),p_amount:Number(amount||0),p_reason:String(reason||'').trim()||null})});
}
export async function tmReloadYijiaPayWallet(phone,saleId,amount,description='億家Pay現金儲值'){
  const c=getCloudConfig();if(!cloudConfigured())throw new Error('Supabase 尚未設定');
  return await request('rpc/tm_reload_yijiapay_wallet',{method:'POST',body:JSON.stringify({p_phone:String(phone||'').replace(/[^0-9]/g,''),p_store_code:String(c.storeId||'001').trim()||'001',p_tm_sale_id:String(saleId||'').trim(),p_amount:Number(amount||0),p_description:String(description||'').trim()||null})});
}
